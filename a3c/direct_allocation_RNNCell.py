...
class direct_allocation_RNNCell(rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        output_fc1 = output_fc1 + tf.constant([[0.1]], dtype = tf.float32, shape = [1,allo_dim])
    ...
    def _fc_variable(self, weight_shape):
        with variable_scope.variable_scope('FC_var') as vs:
            ...
            #bias = tf.Variable(tf.random_uniform(bias_shape, minval=-d, maxval=d), name='bias',initializer=init_ops.constant_initializer(0, dtype=tf.float32)))
